...
destruct_esqlite_connection(ErlNifEnv *env, void *arg)
{
    ...
    printf("destruct_esqlite_connection conn = %016lx queue = %016lx\n", (unsigned long) db, (unsigned long) db->commands);
     ...
     */
    printf("destruct_esqlite_connection conn = %016lx queue = %016lx waiting for cmd thread %016lx to finish\n", (unsigned long) db, (unsigned long) db->commands, (unsigned long) db->tid);
...
destruct_esqlite_statement(ErlNifEnv *env, void *arg)
{
    ...
    printf("destruct_esqlite_statement release_resource conn = %016lx\n", (unsigned long) stmt->connection);
...
esqlite_prepare(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
     ...
     */
    printf("esqlite_prepare calling keep_resource on conn %016lx\n", (unsigned long) conn);
